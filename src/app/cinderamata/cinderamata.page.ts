...
@Component({
  selector: 'app-cinderamata',
  templateUrl: './cinderamata.page.html',
  styleUrls: ['./cinderamata.page.scss'],
})
export class CinderamataPage implements OnInit {

  public itemsCendramata: any = [];
  public path:any = [];
  public next:any=1;
  keyword :string='';
  selectKabupaten :0;
  ...
  getPencarian(ev:any){
    ...
    if (val && val.trim() != '') {
      ...
      var url = this.serviceComponent.webservice()+'get-cendramata';
      this.http.post(url,postData,requestOptions).subscribe((data) => 
      {
        this.path = this.serviceComponent.path();
        this.itemsCendramata = data['response']['items']['data'];
        },error => {
  ...
  async getData(keyword,kabupaten,lat,long)
  {
    ...
    this.http.post(url,postData,requestOptions).subscribe((data) => 
    {
      ...
      this.path=this.serviceComponent.path();
        this.itemsCendramata = data['response']['items']['data'];
        console.log(this.itemsCendramata);
    },error => {
      loading.dismiss()
      alert('Please check your connection')
    });
  ...
  loadData(event) {
    setTimeout(() => {
      ...
      let headers = new HttpHeaders({});
        ...
        var url=this.serviceComponent.webservice()+'get-event?page='+this.next;
        this.http.post(url,postData,requestOptions).subscribe((data) => {
        this.itemsCendramata=this.itemsCendramata.concat(data);
      });
    }, 500);
  }
