...
@Component({
  selector: 'app-detailrestoran',
  templateUrl: './detailrestoran.page.html',
  styleUrls: ['./detailrestoran.page.scss'],
})
export class DetailrestoranPage implements OnInit {
  slideOpts = {
    speed: 600,
    autoplay: true
  };
  ...
  async getPage(){
    ...
    this.http.post(this.serviceComponent.webservice()+'get-restoran/detail',postData,requestOptions).subscribe(async data => { 
      loading.dismiss();
      ...
      this.menuResto=data['response']['menu'];
      this.ulasan=data['response']['ulasan'];
      this.jarak=data['response']['jarak'];
      console.log(data['response']['galeri']);
  ...
  async presentToastSave(item) {
    ...
    .then((fav)=>{
      this.storage.set('fav_resto',fav);
    })
    const toast = await this.toastController.create({
      message: 'Resto telah ditambahkan ke favorit Anda.',
      duration: 3000,
      cssClass: 'dark-toast',
    });
    toast.present();
  ...
  async presentToastRemove(item) {
    this.storage.get('fav_resto').then((data) => {
      ...
      this.storage.set('fav_resto', toKeep);
    });
    this.fav = false

    const toast = await this.toastController.create({
      message: 'Resto telah dihapus dari favorit Anda.',
      duration: 3000,
      cssClass: 'dark-toast',
    });
    toast.present();
...
checkFav()
  {
    ...
    this.storage.get('fav_resto').then((value)=>{
      if(value !== null)
      {
        for (let i of value) {
          if (i.id == id) {
