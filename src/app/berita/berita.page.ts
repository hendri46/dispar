  ...
  loadData(event) {
    setTimeout(() => {
      ...
      let headers = new HttpHeaders({});
        ...
        var url=this.serviceComponent.webservice()+'get-berita?page='+this.next;
        this.http.post(url,postData,requestOptions).subscribe((data) => {
        this.path=this.serviceComponent.path();
        this.itemsBerita=this.itemsBerita.concat(data);
        console.log(data['response']['items']['data']);
