  ...
  loadData(event) {
    setTimeout(() => {
      ...
      let headers = new HttpHeaders({});
        ...
        this.http.post(url,postData,requestOptions).subscribe((data) => {
        // this.itemsEvent=this.itemsEvent.concat(data);
        this.path = this.serviceComponent.path();
        this.itemsEvent = this.itemsEvent.concat(data['response']['items']['data']);
      });
    }, 500);
  }
