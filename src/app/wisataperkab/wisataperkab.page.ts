...
@Component({
  selector: 'app-wisataperkab',
  templateUrl: './wisataperkab.page.html',
  styleUrls: ['./wisataperkab.page.scss'],
})
export class WisataperkabPage implements OnInit {
  ...
  async getData(keyword,kabupaten,lokasi,lat,long)
  {
    ...
    var url = this.serviceComponent.webservice()+'get-objek-wisata/kabupaten';
    this.http.post(url,postData,requestOptions).subscribe((data) => 
    {
      loading.dismiss()
      this.path = this.serviceComponent.path();
