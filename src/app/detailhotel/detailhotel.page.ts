...
@Component({
  ...
  templateUrl: './detailhotel.page.html',
  styleUrls: ['./detailhotel.page.scss'],
})
export class DetailhotelPage implements OnInit {
  slideOpts = {
    speed: 600,
    autoplay: true
  };
  ...
  async getPage(){
    ...
    this.http.post(this.serviceComponent.webservice()+'get-hotel/detail',postData,requestOptions).subscribe(async data => { 
      loading.dismiss();
  ...
  async presentToastSave(item) {
    ...
    .then((fav)=>{
      this.storage.set('fav_hotel',fav);
    })
    const toast = await this.toastController.create({
      message: 'Hotel telah ditambahkan ke favorit Anda.',
      duration: 3000,
      cssClass: 'dark-toast',
    });
    toast.present();
  ...
  async presentToastRemove(item) {
    this.storage.get('fav_hotel').then((data) => {
      ...
      this.storage.set('fav_hotel', toKeep);
    });
    this.fav = false

    const toast = await this.toastController.create({
      message: 'Hotel telah dihapus dari favorit Anda.',
      duration: 3000,
      cssClass: 'dark-toast',
    });
    toast.present();
  ...
  checkFav()
  {
    ...
    this.storage.get('fav_hotel').then((value)=>{
      if(value !== null)
      {
        for (let i of value) {
          if (i.id == id) {
