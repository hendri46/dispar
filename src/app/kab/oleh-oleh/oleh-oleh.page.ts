...
@Component({
  selector: 'app-oleh-oleh',
  templateUrl: './oleh-oleh.page.html',
  styleUrls: ['./oleh-oleh.page.scss'],
})
export class OlehOlehPage implements OnInit {
  ...
  getPencarian(ev:any){
    ...
    if (val && val.trim() != '') {
      ...
      this.http.post(url,postData,requestOptions).subscribe((data) => 
      {
        ...
        this.itemsWisata = data['response']['detail']['data'];
  ...
  async getData(keyword,kabupaten,lokasi,lat,long)
  {
    ...
    this.http.post(url,postData,requestOptions).subscribe((data) => 
    {
      loading.dismiss()
      this.path = this.serviceComponent.path();
      this.itemKab = data['response']['items_kabupaten'];
